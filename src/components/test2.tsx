...
export default function App() {
  ...
  const prevCompIndex = React.useRef(0);
  ...
  React.useEffect(() => {
    ...
    } else if (compIndex == 2 && (prevCompIndex.current - compIndex) < 0) {
      moonTrailRef.current.handleTrail('moon', 'left');
    }

    console.log(prevCompIndex.current, compIndex);
    if (compIndex === 1) {
      fprops.start(0)
      sprops.start(window.innerWidth <= 661 ? 120 : 100)
      landingRef.current.handleTrail('title', 'left');
      aboutRef.current.handleTrail('title', 'right');
    } else if (compIndex == 2) {
      fprops.start(window.innerWidth <= 661 ? -120 : -100)
      if (compIndex - prevCompIndex.current > 0) {
        sprops.start(0)
        aboutRef.current.handleTrail('title', 'left');
  ...
  const sprops = useSpringValue(compIndex !== 2 ? 0 : 100, {
    config: { tension: 280, friction: 80, precision: 0.0001 }
  })

  const tprops = useSpringValue(compIndex !== 3 ? 0 : 100, {
    config: { tension: 280, friction: 80, precision: 0.0001 }
  })
